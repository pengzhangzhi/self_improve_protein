...
**Exploratory-only:** Teacher quality, hidden pseudo-label error, score/error correlation, test-risk oracle-influence alignment, effective-rank relationships, supervised and top-teacher comparisons, the separately carded no-Hessian result, regularization/locality/PCA sweeps, cross-fitted scores, and results on any assay examined during development. The test-risk oracle uses the hidden test outer gradient with the frozen teacher pseudo-gradient; hidden unlabeled labels are used only for pseudo-label error diagnostics. Random-selection error sensitivity uses the immutable `random_diagnostic_replicates=100` draws. Replicate (r) uses the purpose-separated seed stream `random_diagnostic:{replicate}` and cannot replace or modify the confirmatory `random_selection` draw or any fitted model.

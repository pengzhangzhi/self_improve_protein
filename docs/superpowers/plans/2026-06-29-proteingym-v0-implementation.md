...
`fit_task(FitInputs, Protocol)` returns selected hashes, coefficients, test predictions, protocol/source digests, the `threadpoolctl:blas_threads=1` numerical policy, and non-label diagnostics. Freeze its canonical `expected_fit_digest` immediately before unblinding; callers must never recompute that expected value from the artifact at evaluation time. After the trusted loader completes the verified hash join between hidden outcomes and ordered split hashes, freeze `expected_evaluation_digest` before metric computation. `evaluate_task(FitArtifact, EvaluationLabels, protocol=..., expected_fit_digest=..., expected_evaluation_digest=...)` must verify both external digests and every task claim against the supplied locked protocol before accessing hidden-label values, then add metrics and hidden pseudo-error diagnostics without changing the fit artifact. Reject any method artifact whose selected count, pseudo weight, calibration, source hashes, exact weighted normal equations, or diagnostics differ from protocol. Record regularized normal-matrix spectrum/conditioning separately from `data_numerical_rank`, which is computed on the unregularized weighted data Gram matrix. The analysis-only **test-risk oracle influence** uses the hidden test outer gradient with the frozen teacher pseudo-gradient; it does not replace the pseudo-label with the hidden unlabeled label.

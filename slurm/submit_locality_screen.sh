...
# This conditional card is restricted to the already-unblinded GFP + v0
# working sets. The 26 untouched crossfit working sets are out of scope.
...
export SI_LOCALITY_RESULTS_ROOT="${SI_LOCALITY_RESULTS_ROOT:-${SI_ARTIFACT_ROOT}/exploratory/locality_v1/screen}"
export SI_LOCALITY_AGGREGATE="${SI_LOCALITY_AGGREGATE:-${SI_LOCALITY_RESULTS_ROOT}/aggregate.json}"

cd "$SI_REPO_ROOT"
.venv/bin/python -m self_improve_protein.locality_cli \
...
write_job_manifest() {
    {
        ...
        printf '\n  },\n'
        printf '  "kind": "locality_screen_jobs",\n  "schema_version": 1\n}\n'
    } >"$temporary"
...
task_job="$(submit_job \
    ...
    --array=0-44 \
    slurm/locality_task_array.sbatch)"
write_job_manifest
...
aggregate_job="$(submit_job \
    ...
    slurm/locality_aggregate.sbatch)"
write_job_manifest
